...
def safe_read(path, sheet_name=None):
    """
    Excel dosyasını güvenle okur. .xlsx için openpyxl; .xls için xlrd kullanır.
    sheet_name verilirse önce aynı adla dener, yoksa ilk sayfayı kullanır.
    """
    if not path:
        return None
    try:
...
def find_col(df, names):
    """
    Verilen df içinde names listesindeki olası başlıkları
    büyük/küçük harf ve kısmi eşleşme toleransı ile arar.
    Bulduğu ilk uygun orijinal kolon adını döner.
    """
    if df is None:
        return None
...
def to_num(x):
    """
    Metin içinden sayıları güvenli şekilde ayrıştırır.
    NaN/boş -> 0.0; '1.234,56m' -> 1234.56 gibi dönüşümleri yapar.
    """
    try:
        if pd.isna(x):
            return 0.0
...
def aggregate_add(d, key, amt):
    """
    d sözlüğünde key altında amt kadar toplama yapar (None/boş anahtarları atlar).
    """
    if key is None:
        return
    k = str(key).strip()
...
def get_row_status(row):
    ...
    if abs(diff) < 1e-9 and (s + p + c) > 0:
        return "Tam Mutabakat"
    if p == 0 and c > 0 and s == 0:
        return "Girişsiz Tüketim"
    if s > 0 and stok == 0:
        return "Kayıt Dışı Fiziksel Stok"
    if stok > 0 and s == 0 and p == 0:
        return "Sanal Stok (Fizikte Yok)"
...
def build_single_table(prod, sayim, cons, stok, sayim_meta, uretim_meta, stok_meta, cons_meta):
    ...
    for code in sorted(all_codes):
        # Barkod filtre: '-' içermeli ve 'M' ile başlamamalı
...
def main():
    ...
    if df_u is not None:
        ...
        for _, r in df_u.iterrows():
            aggregate_add(prod, r.get(c_b), r.get(c_m))

    # Sayım toplama
    sayim = {}
    if df_s is not None:
        c_b = find_col(df_s, ['BOBİN', 'BOBIN', 'Barkod'])
        c_m = find_col(df_s, ['METRE', 'Metre'])
        for _, r in df_s.iterrows():
            aggregate_add(sayim, r.get(c_b), r.get(c_m))
    ...
    def extract_meta(df, bar_c, cod_c, des_c):
        ...
        for _, r in df.iterrows():
            ...
            if b:
                if cc:
                    c_m[b] = str(r.get(cc) or '').strip()
                if cd:
                    d_m[b] = str(r.get(cd) or '').strip()
        return c_m, d_m

    s_meta = extract_meta(df_s, ['BOBİN', 'BOBIN', 'Barkod'], ['ÜRÜN KODU', 'URUN KODU'], ['ÜRÜN AÇIKLAMA', 'URUN ACIKLAMA', 'URUN AÇIKLAMA'])
    u_meta = extract_meta(df_u, ['BARKOD'], ['MALZEME NO'], ['MALZEME ADI'])
    st_meta = extract_meta(df_st, ['BARKOD'], ['ÜRÜN KODU', 'URUN KODU'], ['ÜRÜN', 'URUN'])
    # Giriş/Çıkış açıklamalarından hangisi varsa onu kullan
    c_meta = extract_meta(df_t, ['BARKOD'], ['GİRİŞ ÜRÜN KODU', 'GIRIS URUN KODU'], ['GİRİŞ ÜRÜN AÇIKLAMA', 'GIRIS URUN ACIKLAMA', 'ÇIKIŞ ÜRÜN AÇIKLAMA', 'CIKIS URUN ACIKLAMA'])

    # --- Tüketim toplama (GÜNCELLENEN MANTIK) ---
    cons = {}
    if df_t is not None:
        cb = find_col(df_t, ['BARKOD'])

        # Açıklama ve miktar sütunlarını esnek arama ile bul
        c_desc_out = find_col(df_t, ['ÇIKIŞ ÜRÜN AÇIKLAMA', 'ÇIKIŞ ÜRÜN ACIKLAMA', 'CIKIS URUN ACIKLAMA'])
        c_teyit = find_col(df_t, ['TEYİT MİKTARI', 'TEYIT MIKTARI'])
        c_giris_tuketim = find_col(df_t, ['GİRİŞ ÜRÜN TÜKETİM MİKTARI', 'GIRIS URUN TUKETIM MIKTARI'])

        for _, r in df_t.iterrows():
            barkod = r.get(cb)
            desc_raw = r.get(c_desc_out) if c_desc_out else None
            desc = str(desc_raw or '')

            # Koşul: "H " (H + boşluk) veya "DMT" ile başlıyorsa TEYİT MİKTARI; aksi halde GİRİŞ ÜRÜN TÜKETİM MİKTARI
            starts_h_space = desc.upper().startswith('H ')   # Boşluk şartı korunur
            starts_dmt = desc.upper().startswith('DMT')
            use_teyit = starts_h_space or starts_dmt

            if use_teyit:
                miktar = to_num(r.get(c_teyit)) if c_teyit else 0.0
            else:
                miktar = to_num(r.get(c_giris_tuketim)) if c_giris_tuketim else 0.0

            aggregate_add(cons, barkod, miktar)

    # Stok toplama
    stok_map = {}
    if df_st is not None:
        cb = find_col(df_st, ['BARKOD'])
        cm = find_col(df_st, ['MİKTAR', 'MIKTAR'])
        for _, r in df_st.iterrows():
            aggregate_add(stok_map, r.get(cb), r.get(cm))
