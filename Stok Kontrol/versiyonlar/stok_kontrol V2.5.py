...
def main():
    ...
    # Meta veri sözlükleri (Barkod -> Açıklama ve Barkod -> Kod)
    meta_desc, meta_code = {}, {}

    def update_meta(b, code, desc):
        b = str(b).strip()
        if not b: return
        if b not in meta_desc or meta_desc[b] is None: meta_desc[b] = desc
        if b not in meta_code or meta_code[b] is None: meta_code[b] = str(code).strip() if code else None
    ...
    if not df_final.empty:
        # Sayısal değerleri yuvarla (binlik ayıraç için hazırla)
        ...
        with pd.ExcelWriter(out_file, engine='openpyxl') as writer:
            ...
            for col_idx, col_name in enumerate(df_final.columns, 1):
                ...
                # Malzeme Kodu sütununu metin olarak ayarla (başındaki sıfırları korumak için)
                if col_name == 'Malzeme Kodu':
                    for row_idx in range(2, len(df_final) + 2):
                        cell = worksheet.cell(row=row_idx, column=col_idx)
