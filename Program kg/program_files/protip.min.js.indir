...
var interval001 = setInterval(function () {
    if (document.readyState === 'complete') {
        ...
        if (window.location.href.indexOf("MainMenu.aspx") > -1) {
            var Obj = null; // sessionStorage.getItem('hnUserImageS');
            getObjectDB('hnUserImageS', function (d) {
                Obj = d
                if (Obj != null) {
                    var script103 = document.createElement('style');
                    script103.type = 'text/css';
                    script103.innerHTML = Obj;
                    document.head.appendChild(script103);
                }
                else {
                    var script102 = document.createElement('style');
                    script102.type = 'text/css';
                    $.ajax({
                        url: 'hnUserImage.ashx', success: function (data) {
                            script102.innerHTML = data;
                            //sessionStorage.setItem('hnUserImageS', data);
                            addObjectDB('hnUserImageS', data);
                            document.head.appendChild(script102);
                        }
                    });
                }
            });
            ...
            if (typeof deleteObjectDB === "function")
                deleteObjectDB("hnUserImageS");
        }

        if (window.location.href.indexOf("MainMenu.aspx") > -1) {
            var Obj = null;
            getObjectDB('hPersonalImageS', function (d) {
                Obj = d;
                if (Obj != null) {
                    var script203 = document.createElement('style');
                    script203.type = 'text/css';
                    script203.innerHTML = Obj;
                    document.head.appendChild(script203);
                }
                else {
                    var script202 = document.createElement('style');
                    script202.type = 'text/css';
                    $.ajax({
                        url: 'hPersonalImage.ashx', success: function (data) {
                            script202.innerHTML = data;
                            addObjectDB('hPersonalImageS', data);
                            document.head.appendChild(script202);
                        }
                    });
                }
            });
            ...
            if (typeof deleteObjectDB === "function")
                deleteObjectDB("hPersonalImageS");
        }
    }
}, 50);


var lc1 = window.location.pathname;
lc1 = lc1.split('/');
var uRl = 'clindDummy.aspx/createCOLORTR';
if (lc1.length > 3) {
    uRl = '../clindDummy.aspx/createCOLORTR'
}
$.ajax({
    type: "POST",
    async: false,
    ...
    dataType: "json",
    success: function f1(d) {
        S = '\r\n<script type="text/javascript">\r\n' + d.d + '\r\n</script>\r\n';
        $(S).appendTo(document.head);
    }
});



//PageMethod For InterfaceStyles!!
doitonstrt();
function getInSh() {
    var X = sessionStorage.getItem("interfaceINTERFACESHADOW");
    if (X == null) return false;
    if (X == "False") return false;
    return true;
}
function getWhSh() {
    var X = sessionStorage.getItem("interfaceLIGHTTEXTSHADOW");
    if (X == null) return false;
    if (X == "False") return false;
    return true;
}
function getBlSh() {
    var X = sessionStorage.getItem("interfaceDARKTEXTSHADOW");
    if (X == null) return false;
    if (X == "False") return false;
    return true;
}
function getAnim() {
    var X = sessionStorage.getItem("interfaceANIMATION");
    if (X == null) return false;
    if (X == "False") return false;
    return true;
}
function styleAdd(CONTROL, STYLE)
{
    if (CONTROL.classList.contains(STYLE) == false) {
        CONTROL.classList.add(STYLE);
    }
}
function doitonstrt() {
    var lc1 = window.location.pathname;
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: uRl + "PNBS.aspx/interfaceSetting",
        data: "{ SOURCE: '" + window.location.pathname + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: (function f1(d) {
            var X = d.d;
            if (X[1] != "") { sessionStorage.setItem("interfaceUSERNAME", X[1]); }
            if (X[2] != "") { sessionStorage.setItem("interfaceFONTFAMILY", X[2]); }
            if (X[3] != "") { sessionStorage.setItem("interfaceFILENAME", X[3]); }
            if (X[4] != "") { sessionStorage.setItem("interfaceWEIGHT", X[4]); }
            if (X[5] != "") { sessionStorage.setItem("interfaceSTRETCH", X[5]); }
            if (X[6] != "") { sessionStorage.setItem("interfaceINTERFACESHADOW", X[6]); }
            if (X[7] != "") { sessionStorage.setItem("interfaceDARKTEXTSHADOW", X[7]); }
            if (X[8] != "") { sessionStorage.setItem("interfaceLIGHTTEXTSHADOW", X[8]); }
            if (X[9] != "") { sessionStorage.setItem("interfaceANIMATION", X[9]); }
            if (X[10] != "") { sessionStorage.setItem("SOUL", X[10]); }


            var loc = window.location.href.substring(0, window.location.href.indexOf(')/') + 2);
            var css = "@font-face {font-family:'Inconsolata';font-style:normal;font-weight: 200 900; font-stretch:50% 200%;font-display: block;src: url('" + loc + "fonts/" + (X[3] == "" ? "Inconsolata.ttf" : X[3]) + "') format('truetype');}\r\n" +
                "body *{font-family:Inconsolata; font-weight:" + (X[4] == "" ? "400" : X[4]) + ";letter-spacing:0.2px; font-stretch:" + (X[5] == "" ? "75" : X[5]) + "%}",
                head = document.head || document.getElementsByTagName('head')[0],
                style = document.createElement('style');
            style.id = "pageStilAlgo";
            var ela = document.getElementById("pageStilAlgo");
            if (ela != null) { ela.remove(); }
            ...
            if (X[6] != "False") {
                ...
                link.setAttribute("href", loc + "globalstylesSHDRBX.css?ver=1.0003.001.01");
            ...
            if (X[7] != "False") {
                ...
                link.id = "gsSHBL";
                link.setAttribute("rel", "stylesheet");
                //if (typeof BREV !== 'undefined') {
                //    if (BREV == true) {
                //        link.setAttribute("href", "globalstylesSHBLRev.css?ver=1.0003.001.01");
                //    }
                //    else {
                //        link.setAttribute("href", "globalstylesSHBL.css?ver=1.0003.001.01");
                //    }
                //}
                //else {
                link.setAttribute("href", loc + "globalstylesSHBL.css?ver=1.0003.001.01");
                //}
                head.appendChild(link);
            }
            else {
                ...
                link.setAttribute("rel", "stylesheet");
                link.id = "gsSHWHNOBL";
                link.setAttribute("href", loc + "globalstylesSHWHNOBL.css?ver=1.0003.001.01");
                head.appendChild(link);
            }
            $("#gsSHWH").remove();
            $("#gsSHBLNOWH").remove();
            if (X[8] != "False") {
                ...
                link.setAttribute("rel", "stylesheet");
                link.id = "gsSHWH";
                //if (typeof WREV !== 'undefined') {
                //    if (WREV == true) {
                //        link.setAttribute("href", "globalstylesSHWHRev.css?ver=1.0003.001.01");
                //    }
                //    else {
                //        link.setAttribute("href", "globalstylesSHWH.css?ver=1.0003.001.01");
                //    }
                //}
                //else {
                link.setAttribute("href", loc + "globalstylesSHWH.css?ver=1.0003.001.01");
                //}
                head.appendChild(link);
            }
            else {
                ...
                link.setAttribute("rel", "stylesheet");
                link.id = "gsSHBLNOWH";
                link.setAttribute("href", loc + "globalstylesSHBLNOWH.css?ver=1.0003.001.01");
                head.appendChild(link);
            }
            ...
            link.id = "gsNOSH";
            link.setAttribute("rel", "stylesheet");
            link.setAttribute("href", loc + "globalstylesNOSH.css?ver=1.0003.001.01");
            head.appendChild(link);
            var I1 = $("#gsNOSH");
            if (I1.length > 1) {
                $(I1[0]).remove();
