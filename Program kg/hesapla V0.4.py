...
# Tonaj hesaplama fonksiyonu (DEBUG modlu)
def calculate_tonnage(data, debug=False):
    machine_tonnage = {}
    current_machine = None
    processed_machines = set() # İşlenen makineleri takip etmek için

    for i, row in enumerate(data):
        # Hata ayıklama: Satır içeriğini yazdır
        if debug:
            print(f"Satır {i:03d}: {row}") 

        # Makine adını kontrol et (daha esnek)
        if row and row[0] and isinstance(row[0], str) and ("Makine" in row[0]): # "Makine" geçen satırları yakala
            # print(f"Potansiyel makine satırı bulundu: Satır {i}") # Hata ayıklama
            try:
                # Makine adını al (örnek: "Makine： M179" -> "M179")
                # Önce "Makine" kelimesinden sonrasını al
                machine_part = row[0].split("Makine", 1)[1]
                # Ardından boşluklara veya diğer ayırıcılara göre temizle
                current_machine = machine_part.strip("：: ·\t").split()[0] # Hem ： hem :, hem · hem boşluk
                if current_machine: # Eğer makine adı gerçekten varsa
                    if current_machine not in machine_tonnage:
                        machine_tonnage[current_machine] = 0
                        processed_machines.add(current_machine)
                        if debug:
                            print(f"  -> Yeni makine tanımlandı: '{current_machine}'") 
                    # else:
                    #     print(f"  -> Zaten tanımlı makine: '{current_machine}'") # Hata ayıklama
                else:
                   if debug:
                        print(f"  -> Uyarı: Satır {i} - Makine adı boş.")
                   current_machine = None
            except (IndexError, AttributeError) as e:
                if debug:
                    print(f"  -> Uyarı: Satır {i} - Makine adı alınamadı. Hata: {e}. Satır içeriği: '{row[0]}'") 
        ...
        if current_machine and row and len(row) > 5 and row[5] is not None:
            try:
                ...
                if kg_str.lower() in ('', '-', '---', 'kg'): # Boş veya geçersiz değerleri atla
                     # if debug: print(f"  -> Uyarı: Satır {i} - Geçersiz KG değeri (boş/geçersiz): '{row[5]}'") # Hata ayıklama
                     pass
                else:
                    kg = float(kg_str)
                    if kg < 0:
                        # Negatif değerleri de toplamaya devam edebiliriz, kullanıcıya sadece bilgi ver
                        if debug: print(f"  -> Bilgi: Satır {i} - Negatif KG değeri: {kg} ({current_machine})")
                    machine_tonnage[current_machine] += kg
                    if debug: print(f"  -> Satır {i}: {current_machine} için {kg} kg eklendi. Toplam: {machine_tonnage[current_machine]}") # Hata ayıklama
            except ValueError as e:
                # Sayıya çevrilemeyen değerleri atla
                if debug: print(f"  -> Uyarı: Satır {i} - KG değeri sayıya çevrilemedi: '{row[5]}'. Hata: {e}") # Hata ayıklama
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n--- Tonaj Hesaplama Başlıyor ---")
        # DEBUG modu aktif edildi
        tonnage = calculate_tonnage(data, debug=True) # debug=True ile detaylı çıktı al
