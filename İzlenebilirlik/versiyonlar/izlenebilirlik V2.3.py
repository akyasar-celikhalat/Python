...
file_name = 'Tüketim 1.3-12.6.xlsx'
...
product_times = {}
product_processes = {}
product_tuketim = {}  # Tüketim bilgilerini saklamak için sözlük
...
for row in df.values:
    ...
    # Çıkış ürününün makine, zaman, proses ve tüketim bilgilerini kaydet
    if çıkış_barkod not in product_machines:
        ...
        product_tuketim[çıkış_barkod] = tuketim  # İlk tüketim değeri
    else:
        # Eğer çıkış_barkod zaten varsa, tüketim değerini topla
        product_tuketim[çıkış_barkod] += tuketim
...
def build_traceability_tree(product_code, root_code, graph, depth=0, process_chain=None):
    ...
    tuketim = product_tuketim.get(product_code, "")  # Toplanan tüketim bilgisi alındı
    ...
    result.append({
        ...
        'Makine': makine,
        'Tüketim (Kg)': tuketim,  # Toplanan tüketim bilgisi eklendi
...
if trace_data:
    ...
    df_result = df_result[['Barkod', 'Ürün Açıklaması', 'Makine', 'Tüketim (Kg)', 'Oluşturma Zamanı', 'Proses', 'İşlem Döngüsü']]  # Tüketim sütunu eklendi
