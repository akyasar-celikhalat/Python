...
if not os.path.exists(girdi_dosyasi):
    print(f"❌ HATA: {girdi_dosyasi} bulunamadı! Program kapatılıyor.")
else:
    ...
    print("✅ Veri yüklendi. Hazır!")

    while True:
        print("\n" + "="*50)
        print("📊 YENİ RAPORLAMA İŞLEMİ")
        print("👉 Barkodları virgülle ayırarak girin.")
        print("👉 Kapatmak için: 'çıkış', 'kapat' veya 'iptal' yazın.")
        print("="*50)
        
        giris = input("\n📝 Giriş: ").strip()

        # Çıkış kontrolü
        if giris.lower() in ['çıkış', 'kapat', 'iptal', 'exit', 'quit']:
            print("\n👋 Program kapatılıyor. İyi çalışmalar!")
            break

        if not giris:
            print("⚠️ Lütfen bir barkod girin veya kapatmak için 'kapat' yazın.")
            continue
        ...
        if sonuc:
            ...
            # Dosya ismine tarih ve saat ekle (Dosyaların karışmaması için)
            zaman_damgasi = datetime.now().strftime("%H%M%S")
            cikti_dosyasi = os.path.join(script_dizini, f"Rapor_{zaman_damgasi}.xlsx")
            
            rapor_df[kolonlar].to_excel(cikti_dosyasi, index=False)
            
            print(f"\n✅ RAPOR HAZIRLANDI: {cikti_dosyasi}")
            print(f"📊 Toplam Hareket: {len(rapor_df)} satır.")
        else:
            print("\n❌ Girdiğiniz barkodlara ait veri bulunamadı.")

        # İşlem sonrası devam onayı
