    ...
    def find_path(current_barcodes, current_path, visited):
        ...
        for current_barcode in current_barcodes:
            ...
            if not rows_for_output and current_barcode in sap_etiket_map:
                 ...
                 if not urun_aciklamasi or urun_aciklamasi == "nan":
                 ...
                 # Format the cycle path to include process
                 formatted_path = [f"{row.get('PROSES', '?')} ({part})" for part in current_path + [current_barcode]]
            ...
            if rows_for_output:
                ...
                formatted_path = [f"{representative_row.get('PROSES', '?')} ({part})" for part in current_path + [current_barcode]]
                ...
                find_path(input_barcodes, current_path + [current_barcode], visited)
                 ...
                 formatted_path = [f"{proses} ({part})" for part in current_path + [current_barcode]]
                 results.append({
                     "Barkod": current_barcode,
                     "Ürün Açıklaması": urun_aciklamasi,
                     "Makine": makine,
...
try:
    ...
    # Alternatif olarak, sonucu yazdırmayı deneyebilirsiniz
    print("\n--- İşlem sırası ters çevrilmiş sonuç (Yazdırıldı) ---")
    print(df_sonuc_ters.to_string(index=False, justify='center'))
